...
function qualityBadge(quality: string | null): React.CSSProperties & { label: string } {
  switch (quality) {
    case 'High':
      return { label: 'High', background: 'rgba(138,158,140,0.2)', color: '#8A9E8C' }
    case 'Medium':
      return { label: 'Medium', background: 'rgba(201,169,110,0.15)', color: '#C9A96E' }
    case 'Low':
      return { label: 'Low', background: 'rgba(192,97,74,0.15)', color: '#C0614A' }
    default:
      return { label: '—', background: 'rgba(255,255,255,0.05)', color: 'rgba(245,240,232,0.3)' }
  }
}
...
function PasswordScreen({ onSuccess }: { onSuccess: () => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (data.ok) {
      sessionStorage.setItem('designlead_auth', 'true')
      onSuccess()
    } else {
        ...
        >
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div className={shake ? 'dl-shake' : ''}>
              ...
              />
              {error && (
                <p style={{ color: '#C0614A', fontSize: 12, fontFamily: 'var(--font-montserrat)', fontWeight: 200, marginBottom: 12 }}>
                  {error}
                </p>
              )}
            </div>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const ok = sessionStorage.getItem('designlead_auth') === 'true'
    setAuthed(ok)
    setChecking(false)
  }, [])
  ...
  function logout() {
    sessionStorage.removeItem('designlead_auth')
  ...
  async function refreshStyleProfile() {
    ...
    try {
      await fetch('/api/scrape-portfolio', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.error('Refresh style profile failed:', err)
    }
      ...
      >
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 40 }}>
            <div>
              <h1 style={{ fontFamily: 'var(--font-playfair)', fontSize: 28, fontWeight: 400, color: '#F5F0E8', margin: '0 0 6px', letterSpacing: '0.02em' }}>
                Studio dashboard
              </h1>
              {studioName && (
                <p style={{ fontFamily: 'var(--font-montserrat)', fontWeight: 200, fontSize: 12, color: 'rgba(245,240,232,0.35)', letterSpacing: '0.1em', margin: 0 }}>
                  {studioName}
                </p>
              )}
            </div>
          ...
          {loadingData ? (
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 40 }}>
                ...
                <MetricCard label="Renders generated" value={rendered} />
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 700 }}>
                  ...
                  <tbody>
                    {submissions.length === 0 ? (
                      <tr>
                        <td colSpan={7} style={{ ...tdStyle, textAlign: 'center', color: 'rgba(245,240,232,0.2)', padding: '40px' }}>
                          No submissions yet
                        </td>
                      </tr>
                    ) : (
                          ...
                          <tr key={sub.id}>
                            ...
                            <td style={tdStyle}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                ...
                                >
                                  {['New', 'Contacted', 'Converted', 'Not a fit'].map((s) => (
                            ...
                            <td style={tdStyle}>
                              <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                                {sub.brief && (
                                  <button
                                    onClick={() => setBriefModal(sub.brief!)}
                                    style={{
                                      background: 'none',
                                      border: 'none',
                                      fontFamily: 'var(--font-montserrat)',
                                ...
                                {sub.render_status === 'complete' && sub.results_page_token && (
                                  <a
                                    href={`/results/${sub.results_page_token}`}
                                    target="_blank"
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          value={opt.value}
                          checked={checked}
                          onChange={() => updateNotifPref(opt.value)}
