...
import { htmlEscape, sanitizeFromName } from '@/lib/utils'
...
async function sendResultsEmail(params: {
  ...
  if (!process.env.RESEND_API_KEY || !process.env.NEXT_PUBLIC_APP_URL || !fromEmail) return
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;background:#0A0A0A;margin:0;padding:24px;">
  ...
  await resend.emails.send({
    from: `${sanitizeFromName(params.studioName, fromNameFallback)} <${fromEmail}>`,
    to: [params.clientEmail],
    replyTo: params.designerEmail ?? undefined,
...
export async function POST(
  ...
  if (
    params.secret.length !== expectedSecret.length ||
    !crypto.timingSafeEqual(Buffer.from(params.secret), Buffer.from(expectedSecret))
  ) {
  ...
  try {
    ...
    // Idempotency: if this slot is already stored, a duplicate webhook fired — ignore it
    const existing: string[] = submission.render_urls ?? []
    if (existing[index]) {
      console.log(`Webhook duplicate ignored: submissionId=${submissionId} index=${index}`)
      return NextResponse.json({ ok: true })
    }
